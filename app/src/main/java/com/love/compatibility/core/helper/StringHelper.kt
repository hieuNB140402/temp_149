...
import com.love.compatibility.core.custom.text.CustomTypefaceSpan
...
object StringHelper {
    ...
    fun sanitizeFileName(input: String): String {
        return input.replace(Regex("[\\\\/:*?\"<>|#%&+=@]"), "_")
            .replace(Regex("\\s+"), " ") // gộp nhiều khoảng trắng
            .trim()
    }

    fun capitalizeWords(input: String) = input.split(" ").joinToString(" ") { it.replaceFirstChar(Char::titlecase) }
